--[[ Derived stat effects for the IntoStarlight Wits stat ]]
local diminishing_returns = require("/isl/lib/diminishing_returns.lua")

local STAT_NAME = "isl_wits"
local PATH = "/isl/player_stats/effects"

local function handler(entity_id, held_items)
  local config = root.assetJson(PATH.."/wits.config")
  local wits = world.callScriptedEntity(
    entity_id,
    "status.stat",
    STAT_NAME
  )
